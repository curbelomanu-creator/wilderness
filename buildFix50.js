// Wilderness 5.6 - robust build button bridge for touch and desktop
(()=>{
function attach(){
  ...
  const open=e=>{e?.preventDefault?.();e?.stopPropagation?.();const modern=window.WildernessConstruction56;if(modern?.openMenu)modern.openMenu();else api.openMenu();};
  btn.addEventListener('pointerdown',open,{passive:false});
  btn.addEventListener('contextmenu',e=>e.preventDefault());
  btn.setAttribute('aria-label','Construir: campamento, asedio o ritual');
  return true;
}
