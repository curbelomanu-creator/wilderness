// Wilderness 5.12.11 - mandatory nation selection and W6-safe capital spawn
(()=>{
...
const overlay=document.createElement('div');overlay.id='nation51';overlay.innerHTML='<div id="nation51card"><div class="n51head"><small>WILDERNESS 5.12.11</small><h1>ESCOGE TU NACIÓN</h1><p>Tu elección determina tu capital, rey y relaciones iniciales. Comenzarás junto a la entrada principal de la capital, con espacio libre para explorar.</p></div><div id="nation51grid"></div><div id="nation51choice">Selecciona una nación para comenzar.</div><button id="nation51start" disabled>SELECCIONA UNA NACIÓN</button></div>';document.body.appendChild(overlay);
...
async function begin(){if(!S.chosen||S.placing)return;S.placing=true;start.disabled=true;start.textContent='PREPARANDO CAPITAL…';choice.textContent='Cargando el acceso principal y restaurando el mundo…';N.chooseNation(S.chosen,{spawn:false});await waitRelease();await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));const result=window.Wilderness6SpawnSafety51210?.placeAtCapital?.(S.chosen,'nation-selection')||placeAtCapital(S.chosen);N.save();window.WildernessRelease?.save?.(false);close();badge();S.placing=false;if(result&&typeof flash==='function')flash(`${result.c.name} · ${result.f.display} · Rey ${result.f.king}`,4300)}
start.addEventListener('pointerdown',e=>{e.preventDefault();begin()},{passive:false});
if(N.state.selected){badge()}else open();
window.WildernessChooseNation51={version:'5.12.11',state:S,open,select,begin,placeAtCapital,safeSpot,badge};
})();