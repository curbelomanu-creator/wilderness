...
(()=>{
  ...
  function enhance(e){
    if(!e?.actor||done.has(e))return;done.add(e);const a=e.actor,c=palette(e);
    // Broader shoulders and layered tunic panels give characters more believable volume.
    ...
    // Sleeves around articulated arms without replacing the pivots.
    if(e.__arms15){box(e.__arms15.left,.34,.56,.34,c,0,-.12,0);box(e.__arms15.right,.34,.56,.34,c,0,-.12,0)}
    // Sandals/ankle wraps on animated legs.
    ...
    // Head cloth that drapes down the back instead of sticking upward.
    ...
    a.add(layers,head);e.__period42={layers,head};
    // Slightly taller, less squat silhouette while preserving gameplay scale.
    if(e===player)a.scale.set(1.03,1.07,1.03);else a.scale.set(1,1.035,1);
  }
  function all(){const a=[player];if(typeof civilians!=='undefined')a.push(...civilians);if(typeof followers!=='undefined')a.push(...followers);if(typeof enemies!=='undefined')a.push(...enemies);return [...new Set(a)]}
