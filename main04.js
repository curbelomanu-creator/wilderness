...
function updatePlayer(dt) {
  ...
  updateChunks();
  const target = player.mesh.position.clone().add(new THREE.Vector3(0, mounted ? 2.8 : 2.15, 0));
  const distance = mounted ? 12.5 : 11;
  const horizontal = Math.cos(camPitch) * distance;
  const offset = new THREE.Vector3(Math.sin(camYaw) * horizontal, Math.sin(camPitch) * distance + (mounted ? 2.2 : 1.8), Math.cos(camYaw) * horizontal);
  const desired = target.clone().add(offset);
  camera.position.lerp(desired, 1 - Math.pow(.0001, dt)); camera.lookAt(target);
}
function orbitCamera(dx,dy){camYaw-=dx*.006;camPitch=THREE.MathUtils.clamp(camPitch+dy*.005,-.18,1.08)}
...
animate(); addEventListener('resize', () => { camera.aspect = innerWidth / innerHeight; camera.updateProjectionMatrix(); renderer.setSize(innerWidth, innerHeight); renderer.setPixelRatio(Math.min(devicePixelRatio, isMobile ? 1 : 1.5)); }); flash(`Wilderness 3.0 estable · Cámara 360° y movimiento restaurados.`, 4200);