...
function buildChunk(cx, cz) {
  ...
  W.settlementsNearBounds(minX, minX + W.CHUNK_SIZE, minZ, minZ + W.CHUNK_SIZE, 8).forEach(ensureSettlement);
  const caveDef = W.caveDefForChunk(cx, cz); if (caveDef) ensureCave(caveDef);
...
function updateChunks(force = false) {
  ...
  for (let dx = -chunkRadius; dx <= chunkRadius; dx++) for (let dz = -chunkRadius; dz <= chunkRadius; dz++) buildChunk(cx + dx, cz + dz);
...
const roleLabels = { infantry: 'INFANTERÍA', archer: 'ARQUEROS', cavalry: 'CABALLERÍA', all: 'TODOS' };
const formationLabels = { line: 'LÍNEA', column: 'COLUMNA', wedge: 'CUÑA' };
const orderLabels = { follow: 'SEGUIR', hold: 'MANTENER', attack: 'ATACAR', retreat: 'RETIRADA' };

function roleFrom(x, z, salt = 0) {
  const r = W.randAt(x, z, 2600 + salt);
  return r < .57 ? 'infantry' : r < .84 ? 'archer' : 'cavalry';
}

function equipRole(e, role) {
  e.role = role;
  if (e.equipment) e.actor.remove(e.equipment);
  e.equipment = new THREE.Group();
  e.actor.add(e.equipment);
  if (e.cavalryHorse) { e.mesh.remove(e.cavalryHorse); e.cavalryHorse = null; }
  e.actor.position.y = 0;

  const friendlyMat = role === 'archer' ? mats.archer : role === 'cavalry' ? mats.cavalry : mats.infantry;
  const enemyMat = role === 'archer' ? mats.enemyArcher : role === 'cavalry' ? mats.enemyCavalry : mats.enemy;
  e.body.material = e.king ? mats.gold : e.enemy ? enemyMat : friendlyMat;

  if (role === 'infantry' || e.king) {
    const spear = addBox(e.equipment, .11, 2.9, .11, mats.wood, .72, 1.7, 0, false);
    spear.rotation.z = .08;
    const shield = addBox(e.equipment, .65, .95, .18, e.enemy ? mats.enemy : mats.shield, -.66, 1.55, .05, false);
    shield.rotation.z = -.08;
  } else if (role === 'archer') {
    const bowA = addBox(e.equipment, .10, 1.2, .10, mats.bow, .67, 1.65, .05, false); bowA.rotation.z = .38;
    const bowB = addBox(e.equipment, .10, 1.2, .10, mats.bow, .67, 1.65, .05, false); bowB.rotation.z = -.38;
    addBox(e.equipment, .26, 1.15, .26, mats.wood, -.55, 1.65, -.22, false);
  } else if (role === 'cavalry') {
    e.actor.position.y = 1.25;
    const horse = new THREE.Group();
    const hmat = e.enemy ? mats.enemyCavalry : mats.horse;
    addBox(horse, 1.25, .78, 2.15, hmat, 0, .95, 0, false);
    addBox(horse, .62, .62, .62, hmat, 0, 1.27, 1.22, false);
    for (const dx of [-.38, .38]) for (const dz of [-.65, .65]) addBox(horse, .17, .82, .17, hmat, dx, .4, dz, false);
    e.mesh.add(horse); e.cavalryHorse = horse;
    const spear = addBox(e.equipment, .11, 3.1, .11, mats.wood, .72, 1.7, 0, false); spear.rotation.z = .12;
  }
}

function makeHumanoid({ x = 0, z = 0, enemy = false, civilian = false, king = false, home = null, role = null, potentialRole = null } = {}) {
  const g = new THREE.Group();
  const actor = new THREE.Group(); g.add(actor);
  const body = new THREE.Mesh(new THREE.BoxGeometry(1.05, 1.7, .7), king ? mats.gold : enemy ? mats.enemy : civilian ? mats.white : mats.blue);
  body.position.y = 1.6; body.castShadow = !isMobile; actor.add(body);
  const head = new THREE.Mesh(new THREE.BoxGeometry(.78, .78, .78), mats.skin);
  head.position.y = 2.85; head.castShadow = !isMobile; actor.add(head);
  for (const dx of [-.26, .26]) {
    const leg = new THREE.Mesh(new THREE.BoxGeometry(.28, 1, .28), mats.black);
    leg.position.set(dx, .55, 0); leg.castShadow = !isMobile; actor.add(leg);
  }
  g.position.set(x, W.groundY(x, z), z); scene.add(g);
  const e = {
    mesh: g, actor, body, hp: king ? 12 : 5, maxHp: king ? 12 : 5, alive: true,
    enemy, civilian, king, recruited: false, speed: king ? 3.7 : enemy ? 4.1 : 4.5,
    cooldown: 0, home, retreat: 0, role: role || null, potentialRole: potentialRole || roleFrom(x, z),
    order: 'follow', holdPos: null, equipment: null, cavalryHorse: null
  };
  if (role || enemy || king) equipRole(e, king ? 'infantry' : (role || roleFrom(x, z, 7)));
...
function makeAnimal(type, x, z, owned = false) {
  ...
  if (type === 'lion') addBox(g, .96, .96, .52, mats.mane, 0, 1.4, .72, false);
  if (type === 'camel') addBox(g, .85, .65, .85, material, 0, 1.85, -.15, false);
  g.position.set(x, W.groundY(x, z), z); scene.add(g);
  const a = {
...
const player = makeHumanoid({ x: 0, z: 0, role: 'infantry' });
civilians.splice(civilians.indexOf(player), 1);
player.enemy = false; player.civilian = false; player.hp = 14; player.maxHp = 14;
player.mesh.scale.set(1.08, 1.08, 1.08);

function seedRand(def, salt) { return W.hash2i(Math.floor(def.x), Math.floor(def.z), salt); }
function ensureSettlement(def) {
  ...
  generatedSettlements.set(def.id, runtime); return runtime;
}

function buildVillage(def) {
  const group = new THREE.Group(); group.name = `village-${def.name}`; scene.add(group);
  const runtime = { ...def, group, captured: false, discovered: false, defendersRuntime: [], kingRuntime: null };
  ...
  for (let i = 0; i < count; i++) {
    ...
    const x = def.x + Math.cos(a) * r, z = def.z + Math.sin(a) * r, y = W.groundY(x, z);
    addBox(group, 5.5, 3.1, 4.7, mats.adobe, x, y + 1.55, z, false);
    const beam = addBox(group, 2.3, .24, 5, mats.wood, x, y + 3.25, z, false); beam.rotation.y = seedRand(def, 1770 + i) * Math.PI;
  }
  worldBox(1.8, .45, 1.8, mats.dark, def.x, W.groundY(def.x, def.z) + .22, def.z, false);
  const civCount = Math.min(12, Math.max(5, Math.ceil(def.population / 4)));
  for (let i = 0; i < civCount; i++) {
    const x = def.x + (seedRand(def, 1800 + i * 2) - .5) * 26;
    const z = def.z + (seedRand(def, 1801 + i * 2) - .5) * 26;
    makeHumanoid({ x, z, civilian: true, home: runtime, potentialRole: roleFrom(x, z, i) });
  }
  return runtime;
}

function buildCity(def) {
  const group = new THREE.Group(); group.name = `city-${def.name}`; scene.add(group);
  const r = 19 + def.tier * 4, wallH = 4.2 + def.tier * .45, step = 4;
  ...
  for (const [dx, dz] of [[-r, -r], [r, -r], [-r, r], [r, r]]) addBox(group, 4.3, wallH + 2.7, 4.3, mats.adobe2, def.x + dx, W.groundY(def.x + dx, def.z + dz) + (wallH + 2.7) / 2, def.z + dz, false);
  ...
  for (let i = 0; i < houseCount; i++) {
    ...
    addBox(group, 5.6, 3.1, 4.8, i % 3 === 0 ? mats.adobe2 : mats.adobe, x, W.groundY(x, z) + 1.55, z, false);
  }
  ...
  const civCount = Math.min(16, Math.max(7, Math.ceil(def.population / 14)));
  for (let i = 0; i < civCount; i++) {
    const x = def.x + (seedRand(def, 2000 + i * 2) - .5) * (r * 1.25);
    const z = def.z + (seedRand(def, 2001 + i * 2) - .5) * (r * 1.1);
    makeHumanoid({ x, z, civilian: true, home: runtime, potentialRole: roleFrom(x, z, i + 90) });
  }

  const archers = Math.max(2, Math.round(def.defenders * .30));
  const cavalry = def.tier >= 2 ? Math.max(1, Math.round(def.defenders * .12)) : 0;
  for (let i = 0; i < def.defenders; i++) {
    const role = i < archers ? 'archer' : i < archers + cavalry ? 'cavalry' : 'infantry';
    let x, z;
    if (role === 'archer') {
      const side = i % 2 ? -1 : 1;
      x = def.x + (seedRand(def, 2100 + i) - .5) * r * 1.55;
      z = def.z + side * (r - 4);
    } else if (role === 'cavalry') {
      x = def.x + (seedRand(def, 2130 + i) - .5) * 10;
      z = def.z + r + 5 + seedRand(def, 2140 + i) * 6;
    } else {
      x = def.x + (seedRand(def, 2160 + i) - .5) * r * 1.4;
      z = def.z + (seedRand(def, 2180 + i) - .5) * r * 1.25;
    }
    const e = makeHumanoid({ x, z, enemy: true, home: runtime, role });
    runtime.defendersRuntime.push(e);
  }
  runtime.kingRuntime = makeHumanoid({ x: def.x, z: def.z - 5, enemy: true, king: true, home: runtime, role: 'infantry' });
  cities.push(runtime); return runtime;
}

function ensureCave(def) {
  if (generatedCaves.has(def.id)) return;
  const group = new THREE.Group(), y = W.groundY(def.x, def.z);
  for (let i = -2; i <= 2; i++) addBox(group, 2.8, 3.4 + Math.abs(i) * .35, 3, mats.rock, def.x + i * 2.2, y + 1.7, def.z, false);
  const opening = addBox(group, 4.3, 3.4, .45, mats.dark, def.x, y + 1.65, def.z - 1.55, false);
  scene.add(group);
  const cave = { ...def, group, opening, discovered: false }; generatedCaves.set(def.id, cave); caves.push(cave);
}

function spawnChunkWildlife(cx, cz) {
  const key = `${cx},${cz}`; if (wildlifeChunks.has(key)) return; wildlifeChunks.add(key);
  const centerX = cx * W.CHUNK_SIZE + W.CHUNK_SIZE / 2, centerZ = cz * W.CHUNK_SIZE + W.CHUNK_SIZE / 2;
  if (Math.hypot(centerX, centerZ) < 45) return;
  const biome = W.biomeAt(centerX, centerZ), roll = W.hash2i(cx, cz, 2201);
  ...
  if (biome === 'desert' && roll > .78) makeAnimal('camel', centerX + (W.hash2i(cx, cz, 2231) - .5) * 24, centerZ + (W.hash2i(cx, cz, 2232) - .5) * 24, false);
  if ((biome === 'rocky' || biome === 'steppe') && W.hash2i(cx, cz, 2240) > .83) makeAnimal('lion', centerX + (W.hash2i(cx, cz, 2241) - .5) * 25, centerZ + (W.hash2i(cx, cz, 2242) - .5) * 25, false);
}
