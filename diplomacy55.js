...
function doorPair(id,x,z,rot=0,label='Puerta',opening=4.2){
  ...
  const h=5.9,half=opening/2,th=.34;
...
function ensureJerusalem(){if(!J?.gates?.length)return;for(let i=0;i<J.gates.length;i++){const a=J.gates[i],id=`hist-jerusalem:${i}`;if(D.gates.has(id))continue;const g=doorPair(id,a.x,a.z,a.rot||0,a.name||`Puerta ${i+1}`,5.15);g.settlementId='hist-jerusalem';g.faction='judah'}}
...
function ensureSettlementGate(rt){const data=settlementGateData(rt);if(!data)return;const {d,x,z,rot}=data,id=`${d.id}:main`;if(D.gates.has(id))return;const g=doorPair(id,x,z,rot,`Puerta de ${d.name||'la ciudad'}`,4.15);g.settlementId=d.id;g.faction=d.faction||d.nation||rt.faction||rt.nation||null;D.settlements.set(d.id,rt)}
function targetForGate(g){const r=rel(g.faction),siege=D.sieges.has(g.settlementId);return(siege||r==='war'||r==='hostile')?0:1}
